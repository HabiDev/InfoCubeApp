class AssortmentsController < ApplicationController
  ...
  def import
    if params[:file].present?
      ...
      redirect_to assortments_path, notice: t('notice.record_imported')
    else
      redirect_to assortments_path, alert: t('notice.record_imported_error')
    end
  ...
  def set_assortments
    ...
    @pagy, @assortments = pagy(@q.result(disinct: true))
